...
const CategoryFormModal = ({ defaultValues, id, open, setOpen }) => {
  const { control, handleSubmit, reset } = useForm({
    defaultValues,
    resolver: useYupValidationResolver(
      object({ name: string().required("Nama kategori tidak boleh kosong") }),
    ),
  });

  const [createCategory, { isLoading: createLoading }] = useMutationHandler(
    useCreateCategoryMutation,
    {
      success: "Kategori baru berhasil dibuat!",
    },
    () => {
      reset();
      setOpen(false);
  ...
  const [updateCategory, { isLoading: updateLoading }] = useMutationHandler(
    useUpdateCategoryMutation,
    {
      ...
      <form
        onSubmit={handleSubmit((d) => {
          if (defaultValues && id) {
            ...
            createCategory(new URLSearchParams(d));
          }
        })}
      >
